...
nixio=require"nixio"
function index()
entry({"admin","status","onliner"},alias("admin","status","onliner","onliner"),_("在线用户"))
entry({"admin","status","onliner","onliner"},template("onliner/onliner"),_("在线用户"),1)
entry({"admin", "status","onliner","speed"}, template("onliner/display"), _("监视器"), 2)
